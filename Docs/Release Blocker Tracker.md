...
| RB-006 | IAP | Backend receipt validation endpoint and server idempotency are not implemented. | Open | Backend/release owner | Client validation hook now fails closed; add backend endpoint, HTTP transport, and server-side transaction idempotency before enabling production IAP. | Blocks production real-money IAP. |

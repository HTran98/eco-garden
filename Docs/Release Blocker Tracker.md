...
| RB-009 | Art/UI | VFX sprites and SFX are still marked as needing authored assets; first-pass background, board backdrop, and UI icon/skin PNGs are wired but still need visual/device validation. | Open | Art/audio owner | Decide whether remaining VFX/SFX assets are required for first release or accepted as post-release polish, then validate background and board contrast on device. | Blocks presentation/audio signoff if final assets are required. |
...
| RB-010 | Decorations | Decoration purchases have visible cosmetic application and bag activation path. | Closed | Client engineering | Validate cosmetics in Play Mode and Android portrait visual pass with the rest of UI/art signoff. | Decoration ownership and active selection are implemented; remaining risk is covered by visual/device validation. |

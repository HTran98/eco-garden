...
| 2026-05-26 | Codex | Pending commit | Not run - manual Play Mode required | Not run - manual Play Mode required | Not run | PlayMode smoke coverage expanded for Level Select toggle and Result complete/fail panel states; `dotnet build Eco-Garden/EcoGarden.PlayModeTests.csproj --no-restore /p:UseSharedCompilation=false` passed, but Unity Test Runner execution is still required. |

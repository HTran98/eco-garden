...
2026-06-02: Added first-pass cute customer NPC art. Generated and imported `char_customer_01.png` under `Assets/EcoGarden/Art/Characters/Resources/Characters`, and `NpcMovementController` now loads it through `Resources.Load` while preserving the procedural NPC sprite fallback. Runtime assembly builds successfully with `/p:UseSharedCompilation=false`; Unity import and Play Mode visual scale validation remain recommended.

2026-06-02: Refined Delivery and Sell drop-zone presentation. `ui_drop_delivery.png` now presents a basket handoff marker, `ui_drop_sell.png` now presents a flower shop storefront, and `HudSkinController` hides drop-zone text labels while preserving existing hitboxes and drag/drop behavior. Runtime assembly builds successfully with `/p:UseSharedCompilation=false`; Unity import and Play Mode visual scale validation remain recommended.

2026-06-02: Fixed duplicate NPC presentation while preserving delivery animation. `ui_drop_delivery.png` now uses a basket handoff marker instead of a second NPC, and `NpcMovementController` always keeps the world-space customer renderer visible when a delivery target is available. Runtime assembly builds successfully with `/p:UseSharedCompilation=false`; Play Mode validation remains recommended.

2026-06-03: Hardened the delivery NPC visibility fix after Play Mode feedback. Removed the UI-as-NPC visual branch from `NpcMovementController`, so the customer always enters, idles, checks out, and returns through the normal world-space animation path. Runtime assembly builds successfully with `/p:UseSharedCompilation=false`; Play Mode visual validation remains recommended.

2026-06-03: Fixed authored NPC sprite visibility. `NpcMovementController` now explicitly enables its `SpriteRenderer`, raises the sorting order, and applies a larger world scale when the PNG customer sprite loads so the high-PPU authored asset remains visible during delivery animation. Runtime assembly builds successfully with `/p:UseSharedCompilation=false`; Play Mode visual validation remains recommended.

2026-06-03: Updated audio generation prompts for Stable Audio after generated SFX included unwanted voice artifacts. `AUDIO_GENERATION_PROMPTS.md` now includes Stable Audio prompt/negative prompt setup, stronger no-vocal/no-speech constraints, and revised SFX/music prompt wording while preserving existing audio filenames and Unity import targets.

2026-06-03: Refined the Stable Audio ambience prompt after `amb_pond_day_loop_01.wav` generated cleanly with a simpler field-recording style prompt. `AUDIO_GENERATION_PROMPTS.md` now keeps the stricter negative prompt guidance for SFX but recommends the focused natural ambience prompt for pond loops unless the generator reintroduces voice, animals, music, or cinematic pads.

2026-06-03: Reworked `AUDIO_GENERATION_PROMPTS.md` into Gemini-friendly structured prompt briefs after testing showed natural creative fields worked better than mechanical rule blocks. Each audio asset now uses Style, Mood, Main sounds/instruments, Duration, Use case, and Technical notes while preserving the existing filenames and Unity import targets.

2026-06-30: Expanded the Gem IAP catalog with 10-100 Gem packs in 10 Gem steps plus 150, 200, and 250 Gem packs, while preserving the existing 80 and 220 Gem legacy products. Added per-product PNG icons under `Assets/EcoGarden/Art/StoreListing/IAP`, assigned them through `ShopItemDefinition.icon`, updated shop UI rows to prefer authored product icons, added the new store product ids to `IapProductIds`/Unity IAP registration, and wired both release scenes to include the new shop assets. Runtime/editor build verification remains required after Unity imports the new sprites.

2026-07-06: Expanded Shop and Bag cosmetic UX. Shop product rows are taller with larger artwork and wider descriptions, while IAP/Gem rows now show only one Buy action instead of a separate Store price badge. Bag decoration rows now support Equip/Remove, mutually exclusive NPC/background skins, and restoring default visuals. Added authored Merchant and Moon Gardener NPC alpha PNGs plus Crystal Pond and Moon Garden portrait background PNGs as purchasable shop items, with scene catalog references and focused EditMode coverage. Shop NPC/background rows now preview their actual art instead of generic decor icons. Unity import and portrait Play Mode visual validation remain required for final scale/readability.

2026-07-06: Continued portrait gameplay/shop art pass after visual review. Board cells and the board backdrop now render with softer transparency so the portrait background's central pond/platform remains visible as the gameplay surface instead of being covered by an opaque square. The default shop catalog and current release scenes no longer list the old decor products, keeping the Decor tab focused on the new NPC and background cosmetics while preserving legacy decoration ids for save compatibility. Shop rows were enlarged again with bigger item art, description text, price text, and action targets. Runtime, editor, and EditMode test assemblies build successfully with `/p:UseSharedCompilation=false`.

2026-07-06: Tightened portrait board placement against the new background art. Board cells are smaller in the release scenes, portrait camera framing now keeps a larger minimum orthographic size so the board stays inside the central diamond/platform area, and the default board colors use softer pond/platform tones with lower corner opacity to blend into each background's middle art. Runtime, editor, and EditMode test assemblies build successfully with `/p:UseSharedCompilation=false`.

2026-07-07: Reworked the play area into a true diamond/isometric footprint for the portrait background platforms. `BoardView` now supports diamond layout world mapping and matching hit tests so tap/drag still resolves to the correct grid cell while the visual board sits inside the central platform shape. Active background cosmetics now push matching tile palettes into the board: Crystal uses warm stone/cream center colors, Moon uses blue-violet night tones, Sunset uses warmer pond colors, and default returns to the pond palette. Added EditMode coverage for diamond grid/world mapping. Runtime, editor, and EditMode test assemblies build successfully with `/p:UseSharedCompilation=false`.

2026-07-07: Adjusted the playfield presentation after visual review. The release scenes now use a centered square grid inside the background's diamond platform instead of the isometric cell layout, avoiding the skewed/offset look while keeping the smaller board scale and background-matched tile palettes. Empty tile opacity was softened so the platform art remains visible beneath the grid. Runtime, editor, and EditMode test assemblies build successfully with `/p:UseSharedCompilation=false`.

2026-07-07: Reduced the authored board content from 8x8 to 5x5 for the current release levels so the playfield sits more comfortably inside the central background platform. Updated all level ScriptableObjects, the Level 15 test factory, parser/board/ability tests, and the scene BoardView cell scale so the 5x5 grid remains readable while occupying less space overall. Runtime, editor, and EditMode test assemblies build successfully with `/p:UseSharedCompilation=false`.

2026-07-07: Tuned the Crystal Lotus background alignment after visual comparison with the Moon background. `EcoGardenBackgroundController` now supports a per-background world offset, and the Crystal Lotus skin applies a slight downward offset so its central platform lines up with the 5x5 board without moving the Moon/default backgrounds. Runtime, editor, and EditMode test assemblies build successfully with `/p:UseSharedCompilation=false`.

2026-07-07: Updated Bag item presentation to match the enlarged Shop previews. Inventory rows are taller, preview art is framed in a larger badge, and owned NPC/background decorations now use the same character/background sprites as their Shop products instead of generic decor icons. Inventory layout coverage now checks the larger preview width. Runtime, editor, and EditMode test assemblies build successfully with `/p:UseSharedCompilation=false`.

2026-07-24: Renamed the player-facing Unity product to `Lotimori` and added a new lotus-and-pond app icon. The 1024px source is assigned across Android legacy, round, and adaptive icon slots; a separate 512px PNG is available for the Google Play store listing. Unity import and an Android launcher-mask/device check remain recommended.

2026-07-24: Reworked the Lotimori app icon after visual review so it reads as a game rather than a wellness app. The revised art shows a compact garden board, two lotus pieces merging into an upgraded flower, a coin reward, and an order basket while preserving the existing Android icon references and Google Play 512px export.

2026-07-24: Refined the Lotimori icon for Android launcher/startup presentation by removing the inset dark background, extending the pond and garden artwork edge-to-edge, enlarging the merge action, and simplifying secondary props. Disabled the Unity splash logo overlay so it no longer covers the app artwork during startup; the Unity splash screen itself remains enabled with the project background.

2026-07-24: Replaced the old Eco Garden opening art with a new 1080x1920 Lotimori portrait splash showing the branded title and lotus merge scene. Assigned it to both the Android splash texture and Unity portrait splash background, disabled background blur, and retained the previously disabled Unity logo overlay.

2026-07-24: Fixed the remaining startup branding path after device feedback. Disabled the entire Unity splash screen, cleared Unity/Android Player Settings splash-image references, changed the runtime `StartupSplashController` resource from the old Eco Garden artwork to the Lotimori portrait artwork, and removed the two obsolete Eco Garden runtime splash textures. Android may still show its mandatory system launch screen briefly, but it now uses the Lotimori application icon rather than a Unity splash layer.

2026-07-24: Redesigned the default gameplay background `bg_pond_foggy_01` to match the Lotimori visual identity. The new portrait pond uses polished turquoise water, soft morning light, rounded garden foliage, lotus accents, and an edge-positioned bridge while preserving a broad low-detail central platform for the 5x5 board and calm upper space for the HUD. Existing resource paths, scene references, and optional cosmetic backgrounds remain unchanged.

2026-07-24: Fixed the Android adaptive launcher icon after device screenshots showed Unity's default cube over the Lotimori artwork. All six adaptive icon sizes now contain both required texture layers instead of only one; the previously missing layer had fallen back to Unity's default foreground. Round and legacy icon assignments remain unchanged. A clean APK rebuild and uninstall/reinstall are required because Android launchers cache adaptive icon layers.
